...
class DCCRN_EC(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = nn.ModuleList()
        # self.linear = nn.Conv2d(kernel_num[-1]*3,kernel_num[-1],3,1,1)

        self.encoder_farend = nn.ModuleList()
        self.encoder_echo =  nn.ModuleList()
    ...
    def forward(self, inputs,echo,farend,lens=None):
        specs = self.stft(inputs) # [batch,514,length]
        # print('specs',specs.shape)
        farend_spec = self.stft(farend)
        ...
        cspecs = torch.stack([real,farend_real,echo_real, imag,farend_imag,echo_imag ], 1) #batch,6,257,length
        # print('cspecs',cspecs.shape)
        cspecs = cspecs[:, :, 1:]   #batch,2,256,length
        # print(cspecs.shape)
        ...
        # echo_cspec = torch.stack([echo_real,echo_imag],1)
        # echo_cspec = echo_cspec[:,:,1:]
        ...
        for idx, layer in enumerate(self.encoder):
            ...
            out = self.encoder[idx](out)
            # out_far = self.encoder_farend[idx](out_far)
            # out_echo = self.encoder_echo[idx](out_echo)
            # print(idx,' ',out.shape)
            encoder_out.append(out)
            # encoder_out_farend.append(out_far)
            # encoder_out_echo.append(out_echo)
        
        batch_size, channels, dims, lengths = out.size()

        # out_ = torch.cat([out,out_far,out_echo],1)
        # out = self.linear(out_)
...
class DCCRN_NS(nn.Module):
    ...
    def forward(self, pnear,lens=None):
        # specs = self.stft(inputs) # [batch,514,length]
        p_near = self.stft(pnear)

        # real = specs[:, :self.fft_len//2+1] #
        # imag = specs[:, self.fft_len//2+1:]
        p_near_real = p_near[:,:self.fft_len//2+1]
        p_near_image = p_near[:,self.fft_len//2+1:]
        ...
        # spec_mags = torch.sqrt(real**2+imag**2+1e-8)
        # spec_mags = spec_mags
        p_near_mags = torch.sqrt(p_near_real**2+p_near_image**2+1e-8)
        p_near_phase = torch.atan2(p_near_image,p_near_real)
        # spec_phase = torch.atan2(imag, real)
        # spec_phase = spec_phase
        cspecs = torch.stack([p_near_real,p_near_image], 1) #batch,4,257,length
        # print('cspecs',cspecs.shape)
        cspecs = cspecs[:, :, 1:]   #batch,2,256,length
        # print(cspecs.shape)
        ...
        if self.masking_mode == 'E':
            ...
            #mask_mags = torch.clamp_(mask_mags,0,100)
            mask_mags = torch.tanh(mask_mags)
            est_mags = mask_mags*p_near_mags
            est_phase = p_near_phase + mask_phase
            real = est_mags*torch.cos(est_phase)
            imag = est_mags*torch.sin(est_phase)
        elif self.masking_mode == 'C':
            real, imag = real*mask_real-imag*mask_imag, real*mask_imag+imag*mask_real
...
class MT_AEC_NS_Model(nn.Module):
    ...
    def __init__(self,):
        ...
        self.noise_supp = DCCRN_NS(rnn_units=32,masking_mode='E',use_clstm=True,kernel_num=[16,32,32])
    ...
    def forward(self,farend,mic):
        ...
        pecho = self.echopre(mic_,farend_)[1]
        pnear = self.echocancellation(mic_,farend_,pecho)[1]

        p_target = self.noise_supp(pnear)[1]

        p_target = torch.unsqueeze(p_target, dim=1)
        pnear = torch.unsqueeze(pnear,dim=1)
        pecho = torch.unsqueeze(pecho,dim=1)


        return pecho,pnear, p_target
